...
public class FileUtil {
    ...
    private static String PROPERTIES_PATH = "classpath:appconfig.properties";
    ...
    private static final String RESULT_FILE_PATH = "/export/Data/result/";

    public static void writeResultFile(String pin, String key, String jsonStr) {
        writeResultFile(pin, key, Arrays.asList(new String[]{jsonStr}));
    }

    public static void writeResultFile(String pin, String key, List<String> resultList) {
        File resultFile = getResultFile(pin, key);

        try {
            FileUtils.writeLines(resultFile, resultList, true);
        } catch (IOException e) {
            LOGGER.error("保存结果文件失败：{}", e.getMessage(), e);
        }
    }

    public static String readFromResultFile(String pin, String key) {
        File resultFile = getResultFile(pin, key);

        if (resultFile.exists()) {
            try {
                return FileUtils.readFileToString(resultFile);
            } catch (IOException e) {
                LOGGER.error("读取结果文件失败：{}", e.getMessage(), e);
            }
        }
    ...
    public static boolean resultFileExist(String userPin, String key) {
        File resultFile = getResultFile(userPin, key);
        return resultFile.exists();
    }

    public static File getResultFile(String userPin, String key) {
        String filePath = RESULT_FILE_PATH + File.separator + userPin + File.separator;
        return new File(filePath + key);
    }

    /**
     * 读properties文件，得到文件中内容
     *
     * @param key 需要读取的key
     * @return
     * @throws IOException
     */
    public static String readValueForProperties(String key) {
        ResourcePropertySource propertySource2 = null;
        try {
            propertySource2 = new ResourcePropertySource("resource", PROPERTIES_PATH);
        } catch (IOException e) {
            return "";
        }
        Object pValue = propertySource2.getProperty(key);
        if (pValue != null)
...
//		return baseClass.getResource(path).getPath();
    ...
    public static String getAbsolutePath(String path) {
        ...
        String reultPath = url != null ? url.getPath() : classLoader.getResource(StringConstants.BLANK).getPath() + path;
    ...
    public static String getExtension(String fileName) {
        ...
        int index = fileName.lastIndexOf(StringConstants.PERIOD);
        if (index == -1) {
            return StringConstants.BLANK;
        } else {
            String ext = fileName.substring(index + 1);
            //扩展名中不能包含路径相关的符号
            return (ext.contains(String.valueOf(UNIX_SEPARATOR)) || ext.contains(String.valueOf(WINDOWS_SEPARATOR))) ? StringConstants.BLANK : ext;
        }
    }
